...
function guess() {
  ...
  for (i = 0; i < guessed_characters.length; i++){
    if (guessing_charcater == guessed_characters[i]){
      guessed = true;
    }
  }
  if (guessed == true){
    alert("you have allready guessed: " + guessing_charcater);
    document.querySelector("input").value = "";
    guessed = false;
  }
  else {
    if (theWord.indexOf(guessing_charcater) != -1 && guessing_charcater != " "){
    ...
    guessed_characters.push(guessing_charcater);
  }
}
